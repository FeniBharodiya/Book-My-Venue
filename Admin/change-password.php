...
if (strlen($_SESSION['odmsaid'] == 0)) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $A_Password = md5($_POST['currentpassword']);
        $newpassword = md5($_POST['newpassword']);
        $sql = "SELECT ID FROM admin WHERE A_ID=:adminid and A_Password=:cpassword";
        $query = $dbh->prepare($sql);
        $query->bindParam(':adminid', $A_ID, PDO::PARAM_STR);
        $query->bindParam(':cpassword', $A_Password, PDO::PARAM_STR);
        $query->execute();
        $results = $query->fetchAll(PDO::FETCH_OBJ);

        if ($query->rowCount() > 0) {
            echo 'if entered';
            $con = "update admin set A_Password=:newpassword where A_ID=:adminid";
            $chngpwd1 = $dbh->prepare($con);
            $chngpwd1->bindParam(':adminid', $A_ID, PDO::PARAM_STR);
            $chngpwd1->bindParam(':newpassword', $A_Password, PDO::PARAM_STR);
            $chngpwd1->execute();
